      ...
      case 2:
        return (
          <div className={styles.formStep}>
            ...
            <input name="latitude" type="number" value={formData.location.latitude} onChange={handleChange} placeholder="Latitude" />
            <input name="longitude" type="number" value={formData.location.longitude} onChange={handleChange} placeholder="Longitude" />
      ...
      case 3:
        return (
          <div className={styles.formStep}>
            ...
            <input name="budget_min" type="number" value={formData.budget_min} onChange={handleChange} placeholder="Minimum Budget" />
            <input name="budget_max" type="number" value={formData.budget_max} onChange={handleChange} placeholder="Maximum Budget" />
          ...
          <div className={styles.formStep}>
            ...
            {formData.children.map((child, idx) => (
              <div key={idx} className={styles.childForm}>
                <h4>Child {idx + 1}</h4>
                <input name="fullName" value={child.fullName} onChange={(e) => handleChildChange(idx, e)} placeholder="Full Name" className={errors[`child_${idx}_fullName`] ? styles.inputError : ""} />
                {errors[`child_${idx}_fullName`] && <span className={styles.errorText}>{errors[`child_${idx}_fullName`]}</span>}

                <input name="age" type="number" value={child.age} onChange={(e) => handleChildChange(idx, e)} placeholder="Age" className={errors[`child_${idx}_age`] ? styles.inputError : ""} />
                {errors[`child_${idx}_age`] && <span className={styles.errorText}>{errors[`child_${idx}_age`]}</span>}

                <input name="currentGrade" value={child.currentGrade} onChange={(e) => handleChildChange(idx, e)} placeholder="Current Grade" />
                <input name="educationStage" value={child.educationStage} onChange={(e) => handleChildChange(idx, e)} placeholder="Education Stage" />
                <input name="preferredCurriculum" value={child.preferredCurriculum} onChange={(e) => handleChildChange(idx, e)} placeholder="Preferred Curriculum" />
              </div>
            ))}
